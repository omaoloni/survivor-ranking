...
def get_sorted_player_list(players: list[tuple[str, str]], sort_descending: bool) -> list[tuple[str, str]]:
    """ 
...
def get_ranking(players: list[tuple[str, str]]) -> dict[str, str]:
    ...
    player_rankings = { player[0] : rank for rank, player in enumerate(sorted_players, start=1)}

    print(player_rankings)
    return player_rankings


def get_power_ranking(existing_player_standings: dict[str, str], current_player_scores: list[tuple[str, str]]) -> list[Player]:
    """
    ...
    :param current_player_scores are the updated scores for each player as of the current episode
